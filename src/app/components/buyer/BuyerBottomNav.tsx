    ...
    <div className="border-t border-gray-200/70 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/85 flex items-center sticky bottom-0 z-10 shadow-[0_-8px_20px_rgba(0,0,0,0.08)]">
      {NAV_ITEMS.map(item => {
        ...
        return (
          ...
          >
            <div className={`rounded-xl px-3 py-1 transition-all relative ${isActive ? 'bg-orange-50 shadow-sm' : ''}`}>
              <Icon size={22} strokeWidth={isActive ? 2.4 : 1.8} />
              {isCart && cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center" style={{ fontSize: '0.65rem', fontWeight: 700 }}>
