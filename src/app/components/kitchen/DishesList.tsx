      ...
      <div className="bg-orange-500 px-5 pt-10 pb-5">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="bg-white text-orange-500 rounded-full p-2 shadow-md"
          >
            <Plus size={20} />
          </button>
        </div>
        <p className="text-orange-100 mt-1" style={{ fontSize: '0.82rem' }}>{orgDishes.length} items</p>
      </div>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs transition-all ${
                filterCat === cat ? 'bg-orange-500 text-white' : 'bg-white text-stone-500 border border-gray-200'
              }`}
            ...
            {filtered.map(dish => (
              <div key={dish.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0 mr-2">
                      ...
                      <span className="inline-block bg-orange-50 text-orange-600 text-xs px-2 py-0.5 rounded-full mt-0.5" style={{ fontWeight: 500 }}>
                        {dish.category}
                      </span>
                    </div>
                    <p className="text-orange-600 flex-shrink-0" style={{ fontWeight: 700 }}>
                      Rs. {dish.price}
                    </p>
                  </div>
                  <div className="mt-1">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${dish.isAvailable ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => deleteDish(dish.id)} className="text-red-400 hover:text-red-600 p-1">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 350"
                  className={`w-full border-2 rounded-xl px-3 py-2.5 text-sm focus:outline-none bg-gray-50 ${errors.price ? 'border-red-300' : 'border-gray-200 focus:border-orange-400'}`}
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-400 bg-gray-50 resize-none"
                />
              </div>
            </div>
            <button onClick={handleSave}
              className="w-full bg-orange-500 text-white py-3.5 rounded-xl mt-5 hover:bg-orange-600 transition-colors"
