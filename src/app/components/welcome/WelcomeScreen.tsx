...
export function WelcomeScreen() {
  ...
  return (
    <MobileLayout>
      <div className="min-h-full bg-red-600 flex flex-col overflow-y-auto">
            ...
            {showDropdown && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl z-20 overflow-hidden">
                {ROLE_OPTIONS.map(opt => (
                  ...
                  >
                    <span className={selectedRole === opt.value ? 'text-red-500' : 'text-stone-400'}>
                      {opt.icon}
                    </span>
                    <div className="text-left">
                      <p className={selectedRole === opt.value ? 'text-red-600' : 'text-stone-800'} style={{ fontWeight: 600 }}>
                        {opt.label}
                      </p>
